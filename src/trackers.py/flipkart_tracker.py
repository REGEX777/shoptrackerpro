...
def calculate_price_change_rate(product_name):
    ...
    time_difference = (historical_data[-1]['timestamp'] - historical_data[0]['timestamp']).total_seconds() / 3600  # in hours

    if time_difference == 0:
        return None
    
    price_change_rate = (last_price - first_price) / time_difference
...
def analyze_price_trends(product_name):
    ...
    if price_change_rate is not None:
        print(f"Price Change Rate: ₹{price_change_rate:.2f} per hour")
    else:
        print("Not enough data to calculate price change rate.")
